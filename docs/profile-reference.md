...
Generated from the plugin's schema reference (`plugins/oss-maintainer/skills/init/references/profile-schema.md`) and readiness reference. Edit those files, then run `python3 scripts/render_docs.py`.


The single specification of the per-repository profile read by every `oss-maintainer`
skill. `skills/init/scripts/validate_profile.py` enforces it; this file explains it.
